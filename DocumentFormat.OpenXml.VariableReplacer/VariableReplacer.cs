            ...
            string openVariableSymbol = "{$", string closeVariableSymbol = "$}")
        {
            ...
            foreach (KeyValuePair<Guid, ICollection<Text>> variableDescription in variableItems)
            {
                ...
                if (match.Success && match.Groups.Count > 1)
                {
                    ...
                    if (replacers.TryGetValue(variableText, out string variableReplacer) && variableOpenerText != null)
                    {
                        variableOpenerText.Text = variableOpenerText.Text.Replace(openVariableSymbol, variableReplacer);
                    }
                }
            }
        }
